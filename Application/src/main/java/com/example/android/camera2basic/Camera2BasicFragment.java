...
//    private String mCameraId;
    ...
    private int mState2 = STATE_PREVIEW;
    ...
    public void onResume() {
        ...
        if (mTextureView.isAvailable()) {
            openCamera(true, mTextureView.getWidth(), mTextureView.getHeight());
        } else {
    ...
    private void setUpCameraOutputs(boolean isBack, int width, int height) {
        Activity activity = getActivity();
    ...
    private void openCamera(boolean isBack, int width, int height) {
        ...
        setUpCameraOutputs(isBack, width, height);
        configureTransform(isBack, width, height);
        Activity activity = getActivity();
        CameraManager manager = (CameraManager) activity.getSystemService(Context.CAMERA_SERVICE);
        try {
            ...
            String cameraId = manager.getCameraIdList()[isBack ? 0: 1];

            manager.openCamera(cameraId/*mCameraId*/, isBack ? mStateCallback : mStateCallback2, mBackgroundHandler);
        } catch (CameraAccessException e) {
            e.printStackTrace();
    ...
    private void createCameraPreviewSession(boolean isBack) {
        try {
            SurfaceTexture texture = isBack ? mTextureView.getSurfaceTexture() : mTextureView2.getSurfaceTexture();
            assert texture != null;
                ...
                mCameraDevice2.createCaptureSession(Arrays.asList(surface, mImageReader.getSurface()),
                        new CameraCaptureSession.StateCallback() {
                            ...
                            public void onConfigured(@NonNull CameraCaptureSession cameraCaptureSession) {
                                ...
                                try {
                                    // Auto focus should be continuous for camera preview.
                                    mPreviewRequestBuilder2.set(CaptureRequest.CONTROL_AF_MODE,
                                            CaptureRequest.CONTROL_AF_MODE_CONTINUOUS_PICTURE);
                                    // Flash is automatically enabled when necessary.
    ...
    private void configureTransform(boolean isBack, int viewWidth, int viewHeight) {
        Activity activity = getActivity();
        if (isBack && null == mTextureView || !isBack && null == mTextureView2 || null == mPreviewSize || null == activity) {
            return;
        }
